    ...
    renderList = () => {
        return (
            this.state.data.map((u) => {
                return (
                    <TouchableOpacity
                        style={{ backgroundColor: 'white', margin: 20, borderColor: 'gray', borderLeftWidth: 5, padding: 10, }} key={u.id}>
                        <Image style={{ width: '100%', height: 200, marginBottom: 10 }} 
                            source={u.image } />
                        <Text style={{ fontSize: 15, fontWeight: "700", marginBottom: 10 }}>{u.title}</Text>
                        <Text style={{ marginEnd: 10, textAlign: 'justify'}}>{u.text}</Text>                        
                    </TouchableOpacity>
                )
            })
        )
        ...
        return (
            <ScrollView style={{ backgroundColor: 'lightblue' }}
                onScroll={({ nativeEvent }) => {
                    if (this.isCloseToBottom(nativeEvent) && this.state.hasMore) { this.getListOfData() }}}>                       
                {this.renderList()}
            </ScrollView>
        )
    }
